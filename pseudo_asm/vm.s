...
;    Updated: 2015/12/27 17:09:32 by ngoguey          ###   ########.fr        ;
;                                                                              ;
;******************************************************************************;

is_state:
	init_state_begin				[+]		jmp is_transi				R
	''								[=]		jmp skip_buffer				R
...
is_transi:
	...
	init_state_transi_flag			[u]		jmp is_transi				R

state_tail:
	init_state_id					[1]		rep							R
	''								[=]		ni							R
	init_state_final				[01]	ni							R
	init_state_flag					[u]		jmp is_state				R
...
skip_char_right:
	...
	init_tape_spacer_right			[-]		jmp skip_char_right			R
	''								[=]		ni							R
	init_tape_isblank				[.]		ni							L
	init_tape_skip_endspacer		[=]		ni							L
skip_char_left:
	init_tape_char_left				[ANY]	ni							L
	init_tape_spacer_left			[-]		jmp skip_char_left			L
	''								[=]		ni							R

head_on_head:
	load_creg2_start				[ANY]	ni{fork}					L
load_creg2_skipspacer:
	load_creg2_{}_skipspacer		[=]		jmp load_creg2_drop			L
	''								[+]		ni							L
	load_creg2_{}_skipchar			[ANY]	jmp load_creg2_skipspacer	L
load_creg2_drop:

...
(*   Updated: 2016/01/30 19:29:01 by ngoguey          ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)
...
let rec loop results db alpha str strlen =
  incr i;
  (* Printf.eprintf "\"%s\"\n%!" (Bytes.to_string str); *)
  let membership = Verifier.verify db (Bytes.to_string str) in
  (match membership with
   | Verifier.Member steps ->
	  let (prev, _) = CA.unsafe_get results strlen in
	  if steps > prev
	  then CA.unsafe_set results strlen (steps, (Bytes.to_string str));
  (* Printf.eprintf "\"%s\" %d \n%!" str steps *)
	  ()
   | _ -> ()
  );
...
let compute db =
  ...
  loop results db alpha (Bytes.make (maxstrlen + 1) db.PD.blank) 0;
